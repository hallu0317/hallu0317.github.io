...
## AWS SAA-C03 ExamTopics 문제 정리(1~10)
...
#### (6) B, Snowball Edge 는 온프레미스의 대량의 데이터를 S3로 이전하는데 좋고, 네트워크가 아닌 디스크나 스토리지에 저장하여 물리적으로 전달

> S3 File Gateway도 온프레미스의 데이터를 s3로 이전하는데 좋지만 네트워크를 사용함.
...
#### (8) B, SQS는 워크로드의 확장,분리 기능 제공

> CloudTrail은 운영, 감사, 규정 준수를 활성화하는 서비스(기록)
...
#### (9) B, S3 Glacier Deep Archive는 다량의 중요한 데이터를 수집,저장하고 이후로 접근하지 않는 경우에 데이터를 보존해둠.
...
#### (10) B, FIFO SQS QUEUE는 메시지가 수신된 순서대로 처리한다.

> standard SQS는 순서없이 전송한다.
